    ...
    func makeRequest(with config: NetworkConfig) -> Result<URLRequest, NetworkError> {
        return config.environment(for: domain)                       // Result<NetworkEnvironmentProviding, NetworkError>
            ...
            .flatMap { (url, env) -> Result<URLRequest, NetworkError> in
                ...
                // Apply environment headers first (can be overridden by route headers)
                env.headers.forEach { key, value in
                    request.setValue(value, forHTTPHeaderField: key)
                }

                // Apply API key from environment if available
                if let apiKey = env.apiKey {
                    request.setValue(apiKey, forHTTPHeaderField: "X-API-Key")
                }

                // Apply route-specific headers last (these override environment headers)
